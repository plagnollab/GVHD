...
########## list of DE genes
DE <- read.csv(paste("/cluster/project8/vyp/Winship_GVHD/claire/results/mhc1_ko/DE_", choice, ext,  ".csv", sep = ""), sep = ",", header = TRUE)
...
nonDE <- read.csv(paste("/cluster/project8/vyp/Winship_GVHD/claire/results/mhc1_ko/non_DE_", choice, ext, ".csv", sep = ""), sep = ",", header = TRUE)
...
write.table(table.results, file = paste("array_analysis/MHC1_KO/results/immgen_association/", choice, "_ImmGen_coarse__module_association_Fishers_exact_test_Pvalues.txt", sep = ""), sep = "\t")
...
write.table(final.table, file = paste("array_analysis/MHC1_KO/results/immgen_association/", choice, "_final_table_coarse.txt", sep = ""), sep = "\t")
...
pdf(paste("/cluster/project8/vyp/Winship_GVHD/claire/results/mhc1_ko/figs/", choice, "_coarse_module_association_graph.pdf", sep = ""))

plot(test1, type = "n", main = paste("Fisher's Exact Test p-values for association \n of the ", choice, " differential gene \n expression dataset with the ImmGen coarse modules", sep = ""), xlab = "Coarse module number", ylab = "-log10(p-value)", xlim = c(xmin, xmax), ylim = c(ymin, ymax), xaxs = "i", yaxs = "i")
